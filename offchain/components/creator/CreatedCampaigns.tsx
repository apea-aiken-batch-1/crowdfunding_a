...
export default function CreatedCampaigns(props: {
  ...
  finishCampaign: (params: any) => Promise<void>;
  cancelCampaign: (params: any) => Promise<void>;
  onError: (error: any) => void;
}) {
  const { lucid, address, crowdfundingAddress, finishCampaign, cancelCampaign, onError } = props;
        ...
        {campaigns?.map(({ campaign, utxo }, c) => (
          <TableRow key={`campaign.${c}`}>
            ...
            <TableCell className="text-center">
              <Button
                onClick={() => (calcCurrentFunds(campaign.backers) >= campaign.goal ? finishCampaign({ utxo }) : cancelCampaign({ campaign, utxo }))}
                className="bg-gradient-to-tr from-pink-500 to-yellow-500 text-white shadow-lg"
                radius="full"
              >
                {calcCurrentFunds(campaign.backers) >= campaign.goal ? "Finish" : "Cancel"}
              </Button>
            </TableCell>
          </TableRow>
        ))}
